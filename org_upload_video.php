...
if(isset($_POST['submit'])){
    ...
    $filedestination='VideoUploads/'.basename($filename);
    if(in_array($fileactualExt,$allowed)) 
	{
		if($fileerror === 0)
		{
			if($filesize < 100000000000000000000) 
			{
				//$filenamenew = uniqid('',true).".".$fileactualExt;
				
				if(move_uploaded_file($filetmpname,$filedestination)){
					$msg="uploaded succesfully";
					$sql="update azura_user set eventvideo='$filename' where role='organizer' and email='".$email."'";
...
<html>
	<head>
		<title>Orgainzer Upload Video</title>
		<meta charset="utf-8" />
					...
					<div class="inner">
						...
						?>
							...
							<div class="flex flex-tabs">
								...
								<div class="tabs">
									<h3>UPLOAD EVENT VIDEO</h3>
								<center>
								<div class="6u 12u$(medium)">
								<form method="post" action="#"  enctype="multipart/form-data">
								<div class="table-wrapper">
								<table><tr><td>
										<td><label><h3>Event Video</h3></label></td>
										<td>
										<div class="6u$ 12u$(xsmall)">
											<input type="file" name="file" required="required" id="file" value="" placeholder="Event Video" />
										</div>
										</td>
										</tr>
										</table>
